...
async def async_setup_entry(
    ...
    pollenStationCode: str | None = (
        config_entry.data.get(CONF_POLLEN_STATION_CODE) or None
    )
...
class SwissWeatherSensor(CoordinatorEntity[SwissWeatherDataCoordinator], SensorEntity):
    ...
    def __init__(
        ...
        self._attr_suggested_object_id = (
            f"{slug(sensor_entry.key)}_{slug(station_name)}"
        )
...
class SwissWeatherWarningCountSensor(
    ...
    def __init__(
        ...
        self._attr_suggested_object_id = (
            f"weather_warning_count_{slug(forecast_name)}"
        )
        self._attr_device_info = device_info
        self._attr_attribution = "Source: MeteoSwiss"
...
class SwissWeatherHighestWarningLevelSensor(
    ...
    def __init__(
        ...
        self._attr_suggested_object_id = (
            f"highest_weather_warning_level_{slug(forecast_name)}"
        )
        self._attr_device_info = device_info
        self._attr_attribution = "Source: MeteoSwiss"
        self._attr_suggested_display_precision = 0
...
class SwissWeatherWarningSlotSensor(
    ...
    def __init__(
        ...
        self._attr_suggested_object_id = (
            f"{slot_label}_weather_warning_{slug(forecast_name)}"
        )
        self._attr_device_info = device_info
        self._attr_attribution = "Source: MeteoSwiss"
...
class SwissPollenLevelSensor(CoordinatorEntity[SwissPollenDataCoordinator], SensorEntity):
    ...
    def __init__(
        ...
        self._attr_suggested_object_id = (
            f"{slug(sensor_entry.key)}_level_{slug(station_name)}"
        )
        self._attr_device_info = device_info
        self._attr_options = [
            PollenLevel.NONE,
